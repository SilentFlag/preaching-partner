...
pub async fn sync_with_server(db: MyDatabase, sync_info: SyncInformation) -> Result<(), DbError> {
    ...
    // Sync Congregations
    let _cong_sync_result = sync_congregations(congregations, db.clone()).await;

    // Sync Categories
    let _category_sync_result = sync_categories(categories, db.clone()).await;

    // Sync Service Groups
    let _service_group_result = sync_service_groups(service_groups, db.clone()).await;
...
async fn _sync_users(groups: Vec<GroupDetails>, db: MyDatabase) -> Result<(), DbError> {
    for group in groups {
        let group_result = db.get_service_group(group.id).await;
        match group_result {
            Ok(_) => {
